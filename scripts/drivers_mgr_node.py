    ...
    for nex_name in self.drivers_active_list:
      ...
      if discovery_method == 'AUTO' and discovery_process == "LAUNCH":
        ...
        if discovery_node_name not in node_list:
          ...
          #Try and launch node
          self.publishMsg("Launching discovery process: " + discovery_node_name)
      ...
      if discovery_method == 'AUTO' and discovery_process == "CALL":
        if discovery_class_name not in self.imported_classes_dict.keys():
          ...
          if success:
            self.publishMsg("Instantiating discovery class " + discovery_class_name + " with nex_dict " + str(nex_dict), level = "WARN")
            discovery_class = imported_class(nex_dict)
          ...
          self.active_paths_list = discovery_class.discoveryFunction(available_paths_list, self.active_paths_list, self.base_namespace)
